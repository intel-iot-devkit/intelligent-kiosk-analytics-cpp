...
BaseDetection::BaseDetection(std::string topoName,
                             ...
                             int maxBatch, bool isBatchDynamic, int isAsync,
                             bool doRawOutputMessages)
    : plugin(), topoName(topoName), pathToModel(pathToModel), deviceForInference(deviceForInference),
...
FaceDetection::FaceDetection(const std::string &pathToModel,
    ...
    : BaseDetection("Face Detection", pathToModel, deviceForInference, maxBatch, isBatchDynamic, isAsync,
      doRawOutputMessages), detectionThreshold(detectionThreshold),
...
void FaceDetection::enqueue(const cv::Mat &frame) {
    ...
    nxtrequest = net.CreateInferRequestPtr();
    width = static_cast<float>(frame.cols);
    height = static_cast<float>(frame.rows);
    Blob::Ptr  inputBlob;
    if(isAsync)
       inputBlob = nxtrequest->GetBlob(input);
    else
...
CNNNetwork FaceDetection::read()  {
    ...
    if (outputDims.size() != 4) {
        throw std::logic_error("Face Detection network output dimensions not compatible should be 4, but was " +
...
AgeGenderDetection::AgeGenderDetection(const std::string &pathToModel,
    ...
    : BaseDetection("Age/Gender", pathToModel, deviceForInference, maxBatch, isBatchDynamic, isAsync,
      doRawOutputMessages), enquedFaces(0) {
}
...
void AgeGenderDetection::enqueue(const cv::Mat &face) {
    ...
    if (enquedFaces == maxBatch) {
        slog::warn << "Number of detected faces more than maximum(" << maxBatch <<
                        ") processed by Age/Gender Recognition network" << slog::endl;
        return;
    }
    if (!request) {
        request = net.CreateInferRequestPtr();
...
CNNNetwork AgeGenderDetection::read() {
    ...
    slog::info << "Batch size is set to " << netReader.getNetwork().getBatchSize() <<
                    " for Age/Gender Recognition network" << slog::endl;
...
void HeadPoseDetection::enqueue(const cv::Mat &face) {
    ...
    if (enquedFaces == maxBatch) {
        slog::warn << "Number of detected faces more than maximum(" << maxBatch <<
                        ") processed by Head Pose estimator" << slog::endl;
        return;
    }
    if (!request) {
        request = net.CreateInferRequestPtr();
...
CNNNetwork HeadPoseDetection::read() {
    ...
    netReader.getNetwork().setBatchSize(maxBatch);
    slog::info << "Batch size is set to  " << netReader.getNetwork().getBatchSize() <<
                    " for Head Pose Estimation network" << slog::endl;
    ...
    for (auto && output : outputInfo) {
        ...
        if (layerNames.find(layer->name) == layerNames.end()) {
            throw std::logic_error("Head Pose Estimation network output layer unknown: " + layer->name +
                                   ", should be " + outputAngleR + " or " + outputAngleP + " or " + outputAngleY);
        }
        if (layer->type != "FullyConnected") {
            throw std::logic_error("Head Pose Estimation network output layer (" + layer->name +
        ...
        if (fc->_out_num != 1) {
            throw std::logic_error("Head Pose Estimation network output layer (" + layer->name +
                                   ") has invalid out-size=" + std::to_string(fc->_out_num) + ", should be 1");
        }
...
double CallStat::getSmoothedDuration() {
    ...
    // visualization is not calculated yet.
