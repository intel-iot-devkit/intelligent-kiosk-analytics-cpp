...
EmotionBarVisualizer::EmotionBarVisualizer(std::vector<std::string> const& emotionNames, cv::Size size,
                                     cv::Size padding, double opacity, double textScale, int textThickness):
                                     emotionNames(emotionNames), size(size), padding(padding),
                                     opacity(opacity), textScale(textScale), textThickness(textThickness),
                                     internalPadding(0) {
    auto itMax = std::max_element(emotionNames.begin(), emotionNames.end(), [] (std::string const& lhs,
                         std::string const& rhs) {
        return lhs.length() < rhs.length();
    });

    textSize = cv::getTextSize(*itMax, cv::FONT_HERSHEY_COMPLEX_SMALL, textScale, textThickness, &textBaseline);
    ystep = (emotionNames.size() < 2) ? 0 : (size.height - 2 * padding.height -
                textSize.height) / (emotionNames.size() - 1);
}
...
HeadPoseVisualizer::HeadPoseVisualizer(float scale, cv::Scalar xAxisColor, cv::Scalar yAxisColor, cv::Scalar zAxisColor,
                                        int axisThickness):
                        xAxisColor(xAxisColor), yAxisColor(yAxisColor), zAxisColor(zAxisColor),
                            axisThickness(axisThickness), scale(scale) {
}
...
void Visualizer::drawFace(cv::Mat& img, Face::Ptr f, bool drawEmotionBar) {
    ...
    if (drawEmotionBar) {
        ...
        cv::Point org(dp.cell.x * xstep + leftPadding, imgSize.height - dp.cell.y * ystep - emotionBarSize.height -
                        bottomPadding);
