...
int analysePeople(cv::Mat frame) {
    try {     
        ...
        cv::namedWindow("Audience Analytics", cv::WINDOW_NORMAL );
        ...
        for (auto &result : FaceDetection.results)
        {
            ...
            if (AgeGender.enabled() && i < AgeGender.maxBatch)
            {
                ...
                // Save the count of male and female every 5th frame
                if(frameCount % 5 == 0) {
                    if (AgeGender[i].maleProb > 0.5) {
                        demographics[dataCount].male.count++;
                        int ageRange = GetAgeGroup(AgeGender[i].age);
            ...
            {
                out << (result.label < FaceDetection.labels.size() ? FaceDetection.labels[result.label] : std::string("label #") + std::to_string(result.label))
            ...
            if (HeadPose.enabled() && i < HeadPose.maxBatch)
            {
                ...
                if(frameCount % 5 == 0)
                {
                    if(HeadPose[i].angle_y > -30 && HeadPose[i].angle_y < 30)
                    {
                        out<<" interested ";
            ...
            auto genderColor = (AgeGender.enabled() && (i < AgeGender.maxBatch)) ? ((AgeGender[i].maleProb < 0.5) ? cv::Scalar(147, 20, 255) : cv::Scalar(255, 0, 0)) : cv::Scalar(100, 100, 100);
        ...
        if(frameCount % 5 == 0) 
        {
            demographics[dataCount].peopleCount = FaceDetection.results.size();
            dataCount++;
        }
        if (!FLAGS_no_show)
        {
            cv::imshow("Audience Analytics", frame);
            cv::waitKey(15);
        }
